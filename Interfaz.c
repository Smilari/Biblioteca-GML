...
/** @brief Captura un número entero ingresado por el usuario desde el teclado.
	...
	@param flagTopeCaracteres Un indicador que, si es verdadero (true), permite que la entrada
	      finalice al presionar ENTER. Si es falso (false), la entrada se limita a `cantCaracteres` incluso
...
/** @brief Permite al usuario elegir entre "GUARDAR" o "CANCELAR" y ejecuta la acción correspondiente.
 *	Muestra un submenú con las opciones "GUARDAR" y "CANCELAR", y permite al usuario seleccionar una de ellas.
...
boolean guardarCambios() {
	...
	estado = (estado) ? false : true; // Invierte el estado para reflejar si se seleccionó "GUARDAR".
...
/** @brief Dibuja una línea horizontal de colores personalizados en la pantalla.
	@param bgd El color de fondo de la línea.
	@param txt El color del texto de la línea. */

void lineaDeColor(int bgd, int txt){
	gotoxy(0, getCoordenada().y); // Posiciona el cursor al comienzo de la línea.
	color(bgd, txt);
